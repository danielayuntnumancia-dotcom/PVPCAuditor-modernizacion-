...
- *¿Está mi potencia contratada de ${billData.kwPunta} kW bien dimensionada?*`,
  ...
  const handleSend = async (textToSend?: string) => {
    ...
    try {
      ...
      const localSources = localStorage.getItem('pvpc_sources');
      const localHistory = localStorage.getItem('pvpc_history');
      const sources = localSources ? JSON.parse(localSources) : [];
      const historyEntries = localHistory ? JSON.parse(localHistory) : [];
